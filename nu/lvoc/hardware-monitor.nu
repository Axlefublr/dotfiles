...
print 'time   cpu  gpu    ppt   cpu°  gpu°   cpu*   cha*'
...
| each {
	...
	let cpu_temp = $sensor_output | get coretemp-isa-0000.'Package id 0'.temp1_input | into int | fill -w 2 -a r
	let gpu_temp = $sensor_output | get amdgpu-pci-0300.junction.temp2_input | into int | fill -w 2 -a r
	...
	print $'($timestamp) ($in.1)% ($in.0)% ($ppt)/($ppt_cap)   ($cpu_temp)°   ($gpu_temp)°  ($cpu_fan)*  ($cha_fan)*'
} | ignore
